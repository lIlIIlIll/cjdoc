...
start_ns="$(python3 "${repo_root}/scripts/portable_probe.py" monotonic-ns)"
if [[ -x "${env_runner}" ]]; then
    python3 "${repo_root}/scripts/run_with_peak_memory.py" --output "${work_dir}/cold-peak-kib.txt" -- \
...
end_ns="$(python3 "${repo_root}/scripts/portable_probe.py" monotonic-ns)"
...
if ((peak_kib >= 0 && peak_kib > max_peak_kib)); then
