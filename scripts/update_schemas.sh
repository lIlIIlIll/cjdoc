...
for schema_name in doc-ir doc-ir-v9 diagnostics cfg-matrix search-index symbol-index api-surface api-surface-v1 api-diff documentation-coverage doctest-results; do
    "${binary}" schema "${schema_name}" | tr -d '\r' \
        >"${stage_dir}/${schema_name}.schema.json"
done
